...
// Ignorer les clés de statut (*s*…) et autres métadonnées
if(pid.startsWith(’_’)) return;
if(!Array.isArray(ids)) return;
...
// - BOUCLE PRINCIPALE -
// Stratégie : 2 passes par jour
// Passe A : couvrir le MINIMUM de toutes les plages (priorité absolue)
// Passe B : remplir jusqu’au MAXIMUM si des éducs sont encore disponibles
...
for(const plage of pj){
  ...
  assigned.forEach(e=>{
    ...
    if(isExcl){ planning[ds][statusKey]='forced'; warnings.push(`${ds} · ${plage.nom} : plage refusée assignée à ${e.prenom} ${e.nom}`); }
    else if(isPref) planning[ds][statusKey]='pref';
    else planning[ds][statusKey]='neutral';
  });
...
// - PASSE B : remplir jusqu'au maximum si quota non atteint -
for(const plage of pj){
  if(plage.tous) continue; // déjà géré en passe A
  const nuit   = isNuit(plage);
  const reqMin = Math.max(0, +plage.min || 1);
  const reqMax = Math.max(reqMin, +plage.max || reqMin);
  if(reqMax <= reqMin) continue; // pas de place supplémentaire

  const dejaDans = (planning[ds][plage.id]||[]).map(x=>+x);
  const encore   = reqMax - dejaDans.length;
  if(encore <= 0) continue;

  // Candidats supplémentaires (pas déjà assignés à cette plage)
  const cands = educs
    .filter(e => !dejaDans.includes(e.id) && canWork(e,d,ds,dow,plage,true))
