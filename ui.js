...
safe(‘me-id’,       el => el.value = ‘’);
safe(‘me-title’,    el => el.textContent = ‘Nouvel éducateur’);
safe(‘me-prenom’,   el => el.value = ‘’);
safe(‘me-nom’,      el => el.value = ‘’);
safe(‘me-notes’,    el => el.value = ‘’);
safe(‘me-contrat’,  el => el.value = ‘temps-plein’);
safe(‘me-heures’,   el => el.value = ‘’);
safe(‘me-h-field’,  el => el.style.display = ‘none’);
...
const demandes = [];
[1,2].forEach(i=>{
const jourEl  = document.getElementById(`me-dem-jour-${i}`);
const typeEl  = document.getElementById(`me-dem-type-${i}`);
