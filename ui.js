...
// Ajouts V20 :
//   - renderDiagnostic enrichi (équipes WE + pression trimestrielle)
//   - badge “Nuit Ven” sur les plages détectées
// ============================================================
...
html+=’<div class="page-sub">Structure des rotations WE, nuits vendredi, et détail des contraintes</div>’;
...
html+=`<div class="card" style="margin-bottom:14px"> <div class="card-hd"> <div class="card-title">📅 Rotation WE — ${monthLabel(moisActuel)}</div> <span class="badge b-blue">Équipes tournantes V20</span> </div> <div style="font-size:.78rem;color:var(--ink3);margin-bottom:12px"> Les blocs WE ont été composés avant génération. Sam+Dim = bloc atomique inséparable. </div>`;
...
// Équipe du dernier WE (continuité mois suivant)
if(rotMois.derniereEquipe&&rotMois.derniereEquipe.length){
  html+=`<div class="alert a-info" style="margin-top:8px;font-size:.77rem">
    <strong>Continuité mois suivant :</strong> Ces éducateurs seront en repos sur le 1er WE du mois prochain :
...
html+=`<div class="card" style="margin-bottom:14px"> <div class="card-hd"> <div class="card-title">📊 Pression trimestrielle — T${nTrim}</div> ${estCloture?'<span class="badge b-orange">⚠ Mois de clôture</span>': estDec?'<span class="badge b-red">🎯 Décembre — Remise à zéro</span>': '<span class="badge b-green">En cours</span>'} </div>`;

```
if(estCloture){
  html+=`<div class="alert a-warn" style="margin-bottom:10px;font-size:.78rem">
    Ce mois est un mois de clôture trimestrielle. L'objectif est que chaque éducateur termine entre <strong>-6h et +6h</strong> du solde trimestriel.
    La pression de génération est renforcée pour atteindre cet objectif.
  </div>`;
}

if(Object.keys(soldeTrimPrec).length){
  html+=`<div style="font-size:.78rem;color:var(--ink3);margin-bottom:8px">Soldes cumulés du trimestre précédent :</div>
  <div style="display:flex;flex-wrap:wrap;gap:8px">`;
  educs.forEach(e=>{
    ...
    html+=`<div style="background:var(--surface2);border:1.5px solid ${ok?'var(--green)':Math.abs(s)<=15?'var(--orange)':'var(--red)'};border-radius:8px;padding:8px 12px;text-align:center;min-width:100px">
      ...
      <div style="font-size:.65rem;color:var(--ink3)">${ok?'✅ OK':Math.abs(s)<=15?'⚠ À corriger':'❌ Critique'}</div>
    </div>`;
  });
  html+=`</div>`;
}
...
// (affichée uniquement si un horaire a été généré ce mois)
if(horaire[currentMonth]&&Object.keys(horaire[currentMonth]).length){
html+=`<div class="card" style="margin-bottom:14px"> <div class="card-title" style="margin-bottom:12px">🎯 Trajectoire annuelle</div> <div style="display:flex;flex-wrap:wrap;gap:8px">`;
...
html+=`<div style="background:var(--surface2);border:1.5px solid ${zoneColor};border-radius:8px;padding:8px 12px;min-width:130px"> <div style="font-size:.72rem;font-weight:600">${e.prenom} ${e.nom}</div> <div style="font-size:.65rem;color:var(--ink3)">${e.contrat}</div> <div style="font-family:'Syne',sans-serif;font-weight:800;font-size:.95rem;color:${zoneColor};margin-top:4px">${t.soldeAnnuel>=0?'+':''}${t.soldeAnnuel.toFixed(1)}h</div> <div style="font-size:.65rem;font-weight:700;color:${zoneColor};text-transform:uppercase;letter-spacing:.5px">${t.zone}</div> ${t.irrecuperable?'<div style="font-size:.62rem;color:var(--red);margin-top:2px">⚠ Irrécupérable</div>':''} </div>`;
});
html+=`</div></div>`;
}

// ── Section 4 : Détail contraintes génération ──
const diag=window._lastDiagnostic||[];
if(diag.length){
html+=`<div class="card"> <div class="card-title" style="margin-bottom:12px">🔍 Détail contraintes — dernière génération</div>`;
diag.forEach(function(d){
html+=`<div style="background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:10px;margin-bottom:8px"> <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px"> <div style="font-weight:700;font-size:.82rem">${d.ds} — ${d.plage}</div> <span class="badge ${d.couverte?'b-green':'b-red'}">${d.couverte?'✅ Couverte':'⚠ Non couverte'}</span> </div> <div style="font-size:.76rem">`;
d.details.forEach(function(r){
html+=`<div style="display:flex;align-items:center;gap:8px;padding:3px 0;border-bottom:1px solid var(--border)"> <div class="avatar" style="background:${r.color};width:24px;height:24px;font-size:.6rem;flex-shrink:0">${r.ini}</div> <span style="font-weight:600;min-width:110px;font-size:.78rem">${r.nom}</span> <span style="color:${r.ok?'var(--green)':'var(--red)'};font-size:.76rem">${r.ok?'✅ Assigné':'❌ '+r.raison}</span> </div>`;
});
html+=`</div></div>`;
});
html+=`</div>`;
} else if(!horaire[currentMonth]){
html+=`<div class="empty"><div class="icon">🔍</div><p>Générez un horaire pour voir le diagnostic complet.</p></div>`;
}
...
// Détection automatique nuit vendredi
...
return `<div class="plage-row"> <div class="plage-dot" style="background:${p.color}"></div> <div class="plage-info"> <div class="plage-name"> ${p.nom} ${p.tous?'<span class="badge b-orange" style="font-size:.65rem">Tous requis</span>':''} ${isNuitVen?'<span class="badge b-purple" style="font-size:.65rem">🌙 Nuit Ven. — rotation propre</span>':''} </div> <div class="plage-detail">${p.debut} → ${p.fin} - ${p.dureeH.toFixed(1)}h - min ${p.min} éduc - ${jours}</div> </div> <button class="btn btn-red btn-sm" onclick="delPlage(${p.id})">Suppr.</button> </div>`;
}).join(’’);
}
